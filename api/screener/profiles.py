...
def _defensive(row: dict, profile: str) -> dict:
    ...
    if len(recent10) != 10 or any(value is None for value in recent10):
    ...
    growth = _status(growth10 >= Decimal("33.3333333333") if growth10 is not None else None)

    record = row.get("dividend_record") or {}
    latest_dividend_year = record.get("latest")
    streak_start = record.get("streak_from")
    if latest_dividend_year is None or streak_start is None:
    ...
    result["profile_note"] = (
        "Dividend history is evidence-incomplete unless this dataset itself proves an uninterrupted 20-year record."
    )
    return result
